...
pub struct EvolutionLog {
    ...
    pub troops_paths: HashMap<TroopId, Vec<Cell>>,
    pub buildings_destroyed: Vec<BuildingId>
}

pub fn simulate_attack(simulation_time_seconds: u32, village: &Village, attack_plan: &AttackPlan, scenery: &Scenery) -> AttackSimulationResult {
    ...
    compute_troops_actions(&village, &troops, &mut actions, scenery);
    ...
    while remaining_time > 0.0 {
        ...
        let evolution_time = get_shortest_action_completion_time(&actions, &troops, &buildings_dps, &village).min(remaining_time);
        ...
        if evolution_time != 0.0 {
            ...
            move_troops(&mut actions, &mut troops, &mut evolution_logs, evolution_time);

            for (building_id, dps) in buildings_dps {
                let destroyed = village.damage_building(dps * evolution_time, building_id);
                if destroyed {
...
fn compute_buildings_dps(actions: &[Action], troops: &[Troop]) -> HashMap<BuildingId, f32> {
    ...
    for action in actions {
        if let ActionKind::Attack(attack_action) = &action.kind {
            ...
            match buildings_damage_per_seconds.entry(attack_action.target) {
                ...
                Entry::Vacant(v) => { v.insert(troop_dps); }
            }
        }
    }
...
fn move_troops(actions: &mut [Action], troops: &mut [Troop], logs: &mut EvolutionLog, evolution_time: f32) {
    for action in actions.iter_mut() {
        if let ActionKind::Move(move_action) = &mut action.kind {
            let troop_id = action.troop_id;
            let walked_path = complete_move_action(move_action, &mut troops[troop_id.0], evolution_time);
            logs.troops_paths.insert(troop_id, walked_path);
        }
    }
}

fn compute_troops_actions(village: &Village, troops: &[Troop], actions: &mut Vec<Action>, scenery: &Scenery) {
            ...
            Some(action) => {
                match &action.kind {
                    ...
                    ActionKind::Attack(attack_action) => {
                        if !village.is_building_destroyed(attack_action.target) {
                        ...
                        match move_action {
                            None => {
                                actions.retain(|s| s.troop_id != troop_id)
                            }
                            Some(move_action) => {
                                *action = move_action
                            }
...
fn create_move_action(troop: &Troop, troop_id: TroopId, village: &Village, scenery: &Scenery) -> Option<Action> {
    let (path, building_id) = find_route_to_next_building(troop, village, scenery)?;

    if path.is_empty() {
        panic!("Could not find a path to go to building {}", building_id.0);
    }
...
fn complete_move_action(action: &mut MoveToBuildingAction, troop: &mut Troop, completion_time: f32) -> Vec<Cell> {
...
fn get_shortest_action_completion_time(actions: &[Action], troops: &[Troop], buildings_dps: &HashMap<BuildingId, f32>, village: &Village) -> f32 {
    let mut smallest = f32::MAX;

    for (building, dps) in buildings_dps {
        let building_lifepoints = village.get_building_lifepoints(*building).expect("Cannot damage a building with undefined life points");
    ...
    for action in actions {
        match &action.kind {
            ActionKind::Move(move_action) => {
                let completion_time = get_total_completion_time_to_travel(&move_action.path, &troops[action.troop_id.0]);
...
fn get_total_completion_time_to_travel(path: &[Cell], troop: &Troop) -> f32 {
...
fn follow_path_within_time(path: &[Cell], troop: &Troop, time_limit: f32) -> (Cell, usize, f32) {
    ...
    for pos in path {
        ...
        if distance_traveled > remaining_distance {
            ...
            let final_position = Cell { x: current_pos.x + vector_goal_x as i16, y: current_pos.y + vector_goal_y as i16 };
